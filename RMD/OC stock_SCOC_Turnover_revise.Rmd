...
<font size="4"> 
**Caption:**
Carbon stocks of (a) sediment (b) bacteria (c) meiofauna and (d) macrofauna of GC1 and GS1. The bar chart and error bars show the mean and standard deviation of carbon stocks in different sampling seasons.
...
plot_grid(plots[[1]],plots[[2]],nrow = 2,rel_heights = c(1,1))+theme(panel.background = element_rect(fill = 'white', color = 'white'))
#ggsave("OC_revise.png",width = 18, height =18,dpi = 300)
```

<font size="4"> 
**Caption:**
Mean carbon stocks of GC1 and GS1 in dry season (in ($mg C/m^2$)). 
</font>
...
#####SED#####
SED<-read_xlsx("SEDstock.xlsx") %>% filter(Cruise!="OR1_1114")
GC1<-SED %>% filter(Station=="GC1") %>% data.frame()
GS1<-SED %>% filter(Station=="GS1") %>% data.frame()
...
`%!in%`<- Negate(`%in%`)
#only single layer: all cruises except for "OR1_1102","OR1_1126"
#10*"0-1"=10cm sediment OC
GC1_sing_sum<-subset(GC1, Cruise %!in% multi, Cruise & OC)%>%
  group_by(Cruise) %>% 
  summarize(OC=10*OC)
GC1_SEDsum<-rbind(GC1_sing_sum,GC1_ml_sum)%>% arrange(Cruise)
GC1_SEDsum<-data.frame(GC1_SEDsum,Season=c("AU","AU","SP","SP","AU","SP","AU","SP","SP","AU"),Station="GC1")
...
GS1_SEDsum<-rbind(GS1_sing_sum,GS1_ml_sum) %>% arrange(Cruise)
GS1_SEDsum<-data.frame(GS1_SEDsum,
                       Season=c("AU","SP","SP","AU","AU","SP","SP","AU"),
                       Station="GS1")
SEDsum<-rbind(GC1_SEDsum,GS1_SEDsum)
#calculate mean stock 
df_sed<-data.frame(
...
Season<-data.frame(
  Station=c(rep("GC1",2),rep("GS1",2)),
  Season=c(rep(c("SP","AU"),2)),
  Mean=c(mean(GC1_SP$OC),mean(GC1_AU$OC),
         mean(GS1_SP$OC),mean(GS1_AU$OC)),
  sd=c(sd(GC1_SP$OC),sd(GC1_AU$OC),
       sd(GS1_SP$OC),sd(GS1_AU$OC))
)
...
Mei<-read_xlsx("meio_biomass.xlsx")%>% filter(Cruise!="OR1_1114")
Mei$Season[Mei$Cruise=="OR1_1126"]<-"AU"
Mei$Season[Mei$Cruise%in%c("OR1_1128","OR1_1132")]<-"SP"
...
MAC<-read_xlsx("macro_biomass.xlsx")%>% filter(Cruise!="OR1_1114")
...
GS1_SP<- MAC %>% filter(Season=="SP"&Station=="GS1")
GS1_AU<- MAC %>% filter(Season=="AU"&Station=="GS1")
MAC_point<-rbind(GC1_SP,GC1_AU,GS1_SP,GS1_AU)
Season<-data.frame(
  Station=c(rep("GC1",2),rep("GS1",2)),
  Season=c(rep(c("SP","AU"),2)),
  Mean=c(mean(GC1_SP$OC),mean(GC1_AU$OC),
...
plot_grid(prow, legend, ncol = 1, rel_heights = c(1, .1))+theme(panel.background = element_rect(fill = 'white', color = 'white'))
#ggsave("SCOC_revise.png",width = 18, height =9, dpi=300)
...
to<-data.frame(Station=c("GC1","GC1","GS1","GS1"),
               OCt_TOU=c(df_sed$Mean[1]/df_TOU$Mean[1]/365,
                         df_sed$Mean[1]/(df_lim$mean[9]+df_lim$mean[12]+df_lim$mean[17])/365,
                         df_sed$Mean[2]/df_TOU$Mean[2]/365,
                         df_sed$Mean[2]/(df_lim$mean[29]+df_lim$mean[32]+df_lim$mean[37])/365),
               OCb_DOU=c(df_bac$Mean[1]/df_DOU$Mean[1],
                         df_bac$Mean[1]/df_lim$mean[9],
                         df_bac$Mean[2]/df_DOU$Mean[2],
                         df_bac$Mean[2]/df_lim$mean[29]),
               OCmm_BOU=c((df_mei$Mean[1]+df_mac$Mean[1])/df_BOU$Mean[1],
                          (df_mei$Mean[1]+df_mac$Mean[1])/(df_lim$mean[12]+df_lim$mean[17]),
                          (df_mei$Mean[2]+df_mac$Mean[2])/df_BOU$Mean[2],
                          (df_mei$Mean[2]+df_mac$Mean[2])/(df_lim$mean[32]+df_lim$mean[37])),
